...
b_gender <- 0.03545
rand_b0 <- 1.1486 #kkk
rand_cond <- 0.05244 #jjjj
rand_covar <- -0.2238 #llll
rand_stim <- 0.2424 #xxxxx
resvar <- 2.2181
samples <- 10000
...
s <- matrix(c(1.1486, -0.2238, -0.2238, 0.05244),2,2)
...
for (i in 1:samples) {
  ...
  #get coefficient
  estimate[i] <- fixef(m1)["genderTRUE"]
  #reduced model wihtout interaction
  m2 <- update(m1, . ~ .-gender, REML=F)
  #store p-value
